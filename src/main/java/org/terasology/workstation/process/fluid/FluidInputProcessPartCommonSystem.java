...
public class FluidInputProcessPartCommonSystem extends BaseComponentSystem {
    ...
    public void validateToStartExecution(ProcessEntityIsInvalidToStartEvent event, EntityRef processEntity,
        ...
        for (Map.Entry<Predicate<String>, Float> requiredItem : getInputItems(fluidInputComponent).entrySet()) {
            ...
            for (int slot : WorkstationInventoryUtils.getAssignedInputSlots(event.getWorkstation(),
                    FLUIDINPUTCATEGORY)) {
                String item = FluidUtils.getFluidAt(event.getWorkstation(), slot);
                if (filter.apply(item)) {
                    ProcessEntityIsInvalidForFluidEvent validForFluidEvent =
                            new ProcessEntityIsInvalidForFluidEvent(event.getWorkstation(), slot,
                                    event.getInstigator(), item);
                    ...
                    foundItem = true;
                    float amountToUse = Math.min(remainingToFind, FluidUtils.getFluidAmount(event.getWorkstation(),
                            slot));
        ...
        FluidInputProcessPartSlotAmountsComponent slotAmountsComponent =
                processEntity.getComponent(FluidInputProcessPartSlotAmountsComponent.class);
    ...
    public void startExecution(ProcessEntityStartExecutionEvent event, EntityRef processEntity,
                               FluidInputComponent fluidInputComponent) {
        FluidInputProcessPartSlotAmountsComponent inputItems =
                processEntity.getComponent(FluidInputProcessPartSlotAmountsComponent.class);
        ...
        if (inputItems != null) {
            for (Map.Entry<Integer, Float> slotAmount : inputItems.slotAmounts.entrySet()) {
                ...
                if (!fluidManager.removeFluid(event.getInstigator(), event.getWorkstation(), slotAmount.getKey(),
                        fluid, slotAmount.getValue())) {
    ...
    public void getInputDescriptions(ProcessEntityGetInputDescriptionEvent event, EntityRef processEntity,
        ...
        for (Map.Entry<String, Float> fluidAmount : fluidInputComponent.fluidVolumes.entrySet()) {
            ...
            event.addInputDescription(new ProcessPartDescription(new ResourceUrn(fluidAmount.getKey()),
